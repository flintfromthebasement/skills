...
#   wp-release-watch.sh --test OLD NEW   # force-analyze a known pair (no state change)
#
set -euo pipefail
...
# ── Modes ────────────────────────────────────────────────────────────
MODE="run" DRY="no" TEST_OLD="" TEST_NEW=""
while [ $# -gt 0 ]; do
  case "$1" in
    --dry-run) DRY="yes"; shift ;;
    --init)    MODE="init"; shift ;;
    --test)    MODE="test"; TEST_OLD="${2:-}"; TEST_NEW="${3:-}"; shift 3 || true ;;
    *) die "unknown arg: $1" ;;
  esac
done
...
# All stable tags (X.Y / X.Y.Z; ignore rc/beta/alpha), highest first.
fetch_stable_tags() {
...
max_sev() { # max of two severities
  [ "$(severity_rank "$1")" -ge "$(severity_rank "$2")" ] && echo "$1" || echo "$2"
}

# ── Diff formatting ──────────────────────────────────────────────────
# Extract commit subjects + file patches from a compare JSON, capped at budget.
format_diff() { # $1=compare.json $2=old $3=new → stdout
  local cmp="$1" old="$2" new="$3"
  {
    ...
    jq -c '.files[]?' "$cmp" | while read -r f; do
      local name patch
      name="$(jq -r '.filename' <<<"$f")"
      patch="$(jq -r '.patch // "(no textual patch — binary or too large)"' <<<"$f")"
      if [ "${#patch}" -gt "$MAX_PATCH_CHARS" ]; then
        echo "### FILE: $name — SKIPPED (patch exceeds budget)"
        continue
      fi
      echo "### FILE: $name"
      echo "$patch"
...
---
<A concise summary. Lead with a one-line verdict. Then a bullet per security fix: <vuln class> (<severity>) — <component>: <what/how>. If severity is capped by diff-only visibility, say so in one line and note it needs human follow-up. Cite any exploit sources from the intel. If nothing security-relevant, say so plainly.>
...
analyze() { # $1=old $2=new → sets RESULT_* vars, writes report
  local old="$1" new="$2"
  local cmp_url="https://github.com/$REPO/compare/${old}...${new}"
  local work; work="$(mktemp -d)"
  local cmp_file="$work/compare.json" diff_file="$work/diff.txt" intel="$work/intel.txt" raw="$work/raw.txt"
  ...
  log "Compare fetched: $(jq -r '.files | length' "$cmp_file" 2>/dev/null || echo 0) files changed."
  ...
  build_prompt "$diff_file" "$old" "$new" "$intel" > "$work/prompt.txt"
  if ! run_analyzer < "$work/prompt.txt" > "$raw"; then
    die "analyzer failed for ${old}...${new} (exit $?)"
  fi
  ...
  RESULT_SEV="$(parse_field SEVERITY "$raw")"
  RESULT_SECURITY="$(parse_field SECURITY_RELEASE "$raw")"
  RESULT_REVIEW="$(parse_field NEEDS_HUMAN_REVIEW "$raw")"
  # Defensive max-floor: trust the model's SEVERITY line but never report lower
  # than a component severity we did read.
  RESULT_SEV="$(max_sev "${RESULT_SEV:-none}" "$(max_sev "${RESULT_CODE_SEV:-none}" "${RESULT_CTX_SEV:-none}")")"
  # Summary: everything after the first '---' separator.
  RESULT_SUMMARY="$(sed '1,/^---$/d' "$raw" | sed '/./,$!d')"
  [ -n "$RESULT_SUMMARY" ] || RESULT_SUMMARY="$raw"

  log "Analysis done: severity=$RESULT_SEV (code=${RESULT_CODE_SEV:-?} context=${RESULT_CTX_SEV:-?}) security_release=$RESULT_SECURITY needs_review=$RESULT_REVIEW"
  ...
  local report="$REPORT_DIR/${new}.md"
  {
    ...
    echo
    echo "- **Severity:** $RESULT_SEV (code: ${RESULT_CODE_SEV:-unknown}, intel: ${RESULT_CTX_SEV:-none})"
    echo "- **Security release:** ${RESULT_SECURITY:-unknown}"
    echo "- **Needs human review:** ${RESULT_REVIEW:-unknown}"
    echo "- **Diff:** $cmp_url"
...
notify() { # $1=version
  local ver="$1"
  if [ "$(severity_rank "$RESULT_SEV")" -lt "$(severity_rank "$NOTIFY_MIN_SEVERITY")" ]; then
    log "Severity $RESULT_SEV below NOTIFY_MIN_SEVERITY=$NOTIFY_MIN_SEVERITY — skipping notification (report kept)."
    return 0
  fi

  local msg="$(severity_emoji "$RESULT_SEV") *${REPO#*/} ${ver} released* — security analysis (severity: *$RESULT_SEV*)

$RESULT_SUMMARY"
  [ "$RESULT_REVIEW" = "yes" ] && msg="$msg

⚠️ Needs human follow-up — full impact not confirmable from the diff alone. Verify against advisories/PoCs."
  ...
  if [ -n "${SLACK_WEBHOOK_URL:-}" ]; then
    if curl -sS -X POST -H 'Content-type: application/json' \
      ...
      log "ERROR: Slack webhook post failed — continuing."
    fi
  fi
  ...
  if [ -n "${NOTIFY_CMD:-}" ]; then
    if printf '%s\n' "$msg" | VERSION="$ver" SEVERITY="$RESULT_SEV" \
      ...
      log "ERROR: NOTIFY_CMD failed — continuing."
    fi
  fi
}
...
if [ "$MODE" = "test" ]; then
  ...
  log "TEST MODE: analyzing ${TEST_OLD}...${TEST_NEW}"
...
[ -n "$tags" ] || die "no stable tags returned from GitHub"
latest="$(head -n1 <<<"$tags")"
