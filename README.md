...
| `people-registry` | [`skills/people-registry`](./skills/people-registry/) | Set up a people.json entity-resolution system for agents — aliases + disambiguation hints, optional deep person context files, and the identity-verification rules that keep agents from treating display names as proof of identity. Scaffold via `scripts/setup.sh`; ships templates, a paste-ready ops snippet, and a fictional two-Sams worked example. |
...
| `wp-release-watch` | [`skills/wp-release-watch`](./skills/wp-release-watch/) | Hourly security tripwire for WordPress core releases: polls git tags, diffs each new release, runs an LLM security analysis over the patch + optional web threat intel, writes a markdown report, notifies via Slack webhook or any command. Model- and harness-agnostic. Idempotent setup.sh installs the cron. |
